...
const VehicleCard = ({ vehicle }) => {
    ...
    const hasImage = vehicle.images && vehicle.images.length > 0 && !vehicle.images[0].includes('/vehicles/');
    const imgSrc = hasImage ? (vehicle.images[0].startsWith('/') ? vehicle.images[0] : `/${vehicle.images[0]}`) : null;
            ...
            <div className="glass-card h-full flex flex-col group-hover:-translate-y-1 group-hover:shadow-lg transition-all duration-300">
                ...
                <div className="relative w-full h-44 sm:h-48 overflow-hidden flex-shrink-0 bg-white/10">
                    {imgSrc && !imgError ? (
                        ...
                        <div className={`w-full h-full bg-gradient-to-br ${bgGradient} flex items-center justify-center group-hover:scale-105 transition-transform duration-500`}>
                ...
                <div className="p-4 flex flex-col flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-slate-400 text-[11px] sm:text-xs mb-4 font-medium">
                        <span className="flex items-center gap-1.5 bg-white/10 py-1.5 px-2 rounded-lg border border-white/10">
                            <FiUsers size={12} className="text-slate-400" /> {vehicle.seats} Select
                        </span>
                        <span className="flex items-center gap-1.5 bg-white/10 py-1.5 px-2 rounded-lg border border-white/10">
                            <FiSettings size={12} className="text-slate-400" /> {vehicle.transmission === 'Automatic' ? 'Auto' : 'Manual'}
                        </span>
                        <span className="flex items-center gap-1.5 bg-white/10 py-1.5 px-2 rounded-lg border border-white/10">
                    ...
                    <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-100">
                        <div>
                            ...
                            <span className="text-slate-400 text-xs ml-1 font-medium">/ day</span>
                        </div>
