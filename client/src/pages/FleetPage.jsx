...
const FleetPage = () => {
    ...
    const hasFilters = selectedCategory || selectedTransmission || selectedFuelType || search;
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <aside className={`lg:w-56 flex-shrink-0 ${showFilters ? 'block' : 'hidden lg:block'}`}>
                        <div className="glass-card p-4 lg:sticky lg:top-20">
                            ...
                            {/* Category */}
                            <div className="mb-4">
                                <h4 className="text-xs text-slate-400 font-medium mb-2 uppercase tracking-wider">Category</h4>
                                <div className="space-y-0.5 max-h-60 overflow-y-auto">
                                    <button
                                        onClick={() => { setSelectedCategory(''); setSearchParams({}); }}
                                        className={`w-full text-left px-2.5 py-1.5 rounded-lg text-xs transition-colors ${!selectedCategory ? 'bg-amber-500/20 text-amber-500' : 'text-slate-400 hover:text-white hover:bg-white/10'
                                            }`}
                                    >
                                        All Categories
                                    </button>
                                    {categories.map((cat) => (
                                        <button
                                            key={cat._id}
                                            onClick={() => { setSelectedCategory(cat._id); setSearchParams({ category: cat._id }); }}
                                            className={`w-full text-left px-2.5 py-1.5 rounded-lg text-xs flex items-center justify-between transition-colors ${selectedCategory === cat._id ? 'bg-amber-500/20 text-amber-500' : 'text-slate-400 hover:text-white hover:bg-white/10'
                                                }`}
                                        >
                                            <span>{cat._id}</span>
                            ...
                            {/* Transmission */}
                            <div className="mb-4">
                                <h4 className="text-xs text-slate-400 font-medium mb-2 uppercase tracking-wider">Transmission</h4>
                                <div className="space-y-0.5">
                                    {['', 'Manual', 'Automatic'].map((t) => (
                                        <button
                                            key={t}
                                            onClick={() => setSelectedTransmission(t)}
                                            className={`w-full text-left px-2.5 py-1.5 rounded-lg text-xs transition-colors ${selectedTransmission === t ? 'bg-amber-500/20 text-amber-500' : 'text-slate-400 hover:text-white hover:bg-white/10'
                                                }`}
                                        >
                                            {t || 'All'}
                                        </button>
                            ...
                            {/* Fuel Type */}
                            <div>
                                <h4 className="text-xs text-slate-400 font-medium mb-2 uppercase tracking-wider">Fuel Type</h4>
                                <div className="space-y-0.5">
                                    {['', 'Petrol', 'Diesel', 'Hybrid', 'Electric'].map((f) => (
                                        <button
                                            key={f}
                                            onClick={() => setSelectedFuelType(f)}
                                            className={`w-full text-left px-2.5 py-1.5 rounded-lg text-xs transition-colors ${selectedFuelType === f ? 'bg-amber-500/20 text-amber-500' : 'text-slate-400 hover:text-white hover:bg-white/10'
                                                }`}
                                        >
                                            {f || 'All'}
                                        </button>
                    ...
                    <div className="flex-1 min-w-0">
                        {loading ? (
                            ...
                            <div className="glass-card p-10 text-center">
                                ...
                                <h3 className="text-white text-base font-semibold mb-1">No vehicles found</h3>
