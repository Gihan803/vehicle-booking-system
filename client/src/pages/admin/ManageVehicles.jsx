...
const CATEGORIES = ['Alto', 'WagonR', 'Every', 'Buddy', 'Premio', 'Axio', 'Aqua', 'Prius', 'Van', 'SUV', 'Other'];

const emptyForm = {
    name: '', category: 'Alto', pricePerDay: '', features: '',
    seats: '4', transmission: 'Manual', fuelType: 'Petrol',
    year: '', plateNumber: '', description: '', available: true,
};
            ...
            <div className="container-app py-8">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p className="text-slate-400">{vehicles.length} vehicles in your fleet</p>
                    </div>
                ...
                {loading ? (
                    <div className="space-y-3">
                        {[...Array(5)].map((_, i) => <div key={i} className="glass-card shimmer h-16" />)}
                    </div>
                ) : (
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-white/10">
                                        <th className="text-left py-3 px-4 text-slate-400 text-sm font-medium">Vehicle</th>
                                        <th className="text-left py-3 px-4 text-slate-400 text-sm font-medium hidden sm:table-cell">Category</th>
                                        <th className="text-left py-3 px-4 text-slate-400 text-sm font-medium hidden md:table-cell">Specs</th>
                                        <th className="text-left py-3 px-4 text-slate-400 text-sm font-medium">Price/Day</th>
                                ...
                                <tbody>
                                    {vehicles.map((v) => (
                                        <tr key={v._id} className="border-b border-white/10 hover:bg-white/10 transition-colors">
                                            <td className="py-3 px-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-slate-400 text-xs">{v.plateNumber}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-3 px-4 hidden sm:table-cell">
                                                <span className="px-2 py-1 bg-amber-500/20 text-amber-500 text-xs rounded-md">{v.category}</span>
                                            </td>
                                            <td className="py-3 px-4 hidden md:table-cell">
                                                <div className="flex gap-3 text-slate-400 text-xs">
                                                    <span className="flex items-center gap-1"><FiUsers size={12} /> {v.seats}</span>
                                                    <span className="flex items-center gap-1"><FiSettings size={12} /> {v.transmission}</span>
                                                </div>
                                            </td>
                                            <td className="py-3 px-4">
                                                <span className="text-amber-500 font-semibold text-sm">Rs. {v.pricePerDay?.toLocaleString()}</span>
                                            </td>
                                            <td className="py-3 px-4">
                                                <span className={`badge ${v.available ? 'badge-confirmed' : 'badge-rejected'}`}>
                                                    {v.available ? 'Available' : 'Unavailable'}
                                                </span>
                                            </td>
                                            <td className="py-3 px-4 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    <button onClick={() => openEditModal(v)} className="p-2 text-slate-400 hover:text-white hover:bg-white/10 rounded-lg transition-colors" title="Edit">
                                                        <FiEdit2 size={16} />
                                                    </button>
                                                    <button onClick={() => handleDelete(v._id)} className="p-2 text-slate-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors" title="Delete">
                                                        <FiTrash2 size={16} />
                                                    </button>
                                                </div>
                                            </td>
                    ...
                    <div className="glass-card p-6 w-full max-w-xl my-8 relative">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div>
                                    <label className="block text-sm text-slate-400 mb-1">Price/Day (Rs.) *</label>
                                    <input type="number" value={form.pricePerDay} onChange={(e) => setForm({ ...form, pricePerDay: e.target.value })} className="input-field" required min="0" />
                                </div>
                                <div>
                            ...
                            <div>
                                ...
                                <input type="text" value={form.plateNumber} onChange={(e) => setForm({ ...form, plateNumber: e.target.value })} className="input-field" />
                            </div>

                            <div>
                                <label className="block text-sm text-slate-400 mb-1">Features (comma-separated)</label>
                                <input type="text" value={form.features} onChange={(e) => setForm({ ...form, features: e.target.value })} className="input-field" placeholder="AC, Power Steering, USB" />
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                <input type="checkbox" id="available" checked={form.available} onChange={(e) => setForm({ ...form, available: e.target.checked })} className="w-4 h-4 accent-secondary" />
                                <label htmlFor="available" className="text-sm text-slate-700">Available for booking</label>
                            </div>
