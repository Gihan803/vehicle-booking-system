    ...
    try {
        ...
        if (status === 'confirmed') {
            const overlapping = await Booking.findOne({
                ...
                $or: [
                    { startDate: { $lte: currentBooking.endDate }, endDate: { $gte: currentBooking.startDate } },
                ],
            });
...
router.post('/vehicles', upload.array('images', 5), async (req, res) => {
    try {
        const { name, category, pricePerDay, features, seats, transmission, fuelType, year, plateNumber, description } = req.body;
...
router.put('/vehicles/:id', upload.array('images', 5), async (req, res) => {
    try {
        ...
        const { name, category, pricePerDay, features, seats, transmission, fuelType, year, plateNumber, description, available } = req.body;

        if (name) vehicle.name = name;
        if (category) vehicle.category = category;
        if (pricePerDay) vehicle.pricePerDay = Number(pricePerDay);
        if (features) vehicle.features = typeof features === 'string' ? JSON.parse(features) : features;
